...
model_path = model_dir / "Transformer.keras"
...
<head>
    ...
    <style>
        ...
        .probability { font-size: 16px; margin-top: 10px; }
    </style>
</head>
<body>
    <h1>情感分析系统</h1>
    ...
    <div class="footer">基于Transformer的中文情感分析系统</div>
    ...
    <script>
        document.getElementById('analysisForm').addEventListener('submit', function(e) {
            ...
            .then(data => {
                ...
                probabilityDiv.textContent = '置信度: ' + data.probability;
...
# 构建 Transformer 模型
def build_transformer_model(vocab_size, max_len, embedding_dim=128, num_heads=4, ff_dim=256):
...
def train_model():
    ...
    # 定义 Transformer 模型
    model = build_transformer_model(len(vocab), max_len, embedding_dim=128, num_heads=4, ff_dim=256)
    model.summary()
    ...
    # 模型训练
    model.fit(train_seq_mat, train_y_encoded, batch_size=32, epochs=50,
...
def load_existing_model():
    ...
    if model is None and model_path and model_path.exists():
        ...
        try:
            # 加载模型 - 使用 custom_objects 参数确保自定义层能被正确加载
...
def predict_(text_o):
    ...
    probability = float(t_pre[0][0])  # 获取概率值
    ...
    try:
        ...
        if pred_label == 0:
            sentiment = "negative"
            prob_percentage = (1 - probability) * 100  # 负面概率
        else:
            sentiment = "active"
            prob_percentage = probability * 100  # 正面概率

        return sentiment, f"{prob_percentage:.2f}%"
    except Exception as e:
        print(f"标签映射错误: {e}")
        return "unknown", "N/A"
...
def analyze():
    ...
    text = data.get('text', '')
    if not text:
        return jsonify({'result': '请输入文本喵', 'probability': ''})

    try:
        sentiment, probability = predict_(text)
        return jsonify({
            'result': sentiment,
            'probability': probability
        })
    except Exception as e:
        return jsonify({'result': f'分析出错喵: {str(e)}', 'probability': ''})
